#include "Server.h"
#include "../Controllers/Logs/SocketLogsFilesAction.h"
...
#include "WebSocket/SocketUserData.h"
#include <bitset>
#include <cstring>
...
void Server::processWebSocketReq(const Request& req) {
    ...
    auto webConn = std::make_shared<WebSocket::SocketConnection>(connection);

    for (unsigned i = 0; i < this->_socketRoutes.size(); i++) {

        if (this->_socketRoutes[i].check(req.getUri(), req.getMethod())) {

            action = this->_socketActions[i].getAction(req.getUri(), webConn);
            break;
        }
    }
    ...
    std::string keyResponse = std::string(req.getHeader("Sec-WebSocket-Key")) + "258EAFA5-E914-47DA-95CA-C5AB0DC85B11";
    ...
    auto socketUserData = new WebSocket::SocketUserData(webConn, action);

    this->_socketConnections.insert(std::make_pair(webConn->getFd(), action));

    bufferevent_setcb(connectBuffer, [](struct bufferevent* bev, void* ctx) -> void {
        auto userData = (WebSocket::SocketUserData*)ctx;

        auto conn = userData->getConn();
        ...
        if (reader->isEndRead()) {
            ...
            if (frameData.opcode == WebSocket::Frame::OPCODE_CLOSE) {
                ...
                auto socketAction = userData->getAction();
                socketAction->run(frameData.payload);
                reader->clear();
            }
        }
    },
        nullptr, nullptr, socketUserData);
